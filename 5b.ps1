...
$emptySeats = 0..1023 | Where-Object {$_ -notin $seats}

for ($i = 1; $i -lt ($emptySeats.Length - 1); $i += 1) {
    if (($emptySeats[$i - 1] -ne $emptySeats[$i] - 1) -and ($emptySeats[$i + 1] -ne $emptySeats[$i] + 1)) {
        Write-Output "Your Seat is $($emptySeats[$i])"
    }
}
